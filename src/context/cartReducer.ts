...
export interface CartAccion{
    type: "ADD_TO_CART" | "REMOVE_FROM_CART" | "CLEAR_CART";
    payload: CartProduct
}

export const cartReducer = (state: CartState, action: CartAccion) : CartState => {
    switch(action.type) {
        case "ADD_TO_CART": {
            ...
            if(existeItem) {
                return {
                    ...state,
                    cartItems: state.cartItems.map((item) => item.id === id ? {...existeItem, 
                    quantity: existeItem.quantity + 1 } : item )
                }
            }else {
                return {
                    ...state,
        ...
        case "REMOVE_FROM_CART": {
            ...
            //Validar si el itema ya existe en el carrito
